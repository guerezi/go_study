package usecases

import (
	"context"
	"errors"
	"fmt"
	"imobiliaria/internal/models"
	"imobiliaria/internal/repositories/mocks"
	"testing"

	"github.com/stretchr/testify/assert"
)

// GENERATED BY HENRY. DO NOT EDIT.

func ExampleCreateUser() {
	repo := &mocks.Repositories{}

	ctx := context.Background()
	age := 12 /// https://youtu.be/DM4qDJ9jR0A

	repo.On("CreateUser", ctx, &models.User{
		Name:  "Gabriel",
		Email: "gabriel@gmail.com",
		Age:   &age,
	}).Return(&models.User{
		ID:    0,
		Name:  "Gabriel",
		Email: "gabriel@gmail.com",
		Age:   &age,
	}, nil).Once()

	usecases := usecases{
		repo: repo,
	}

	user := &models.User{
		Name:  "Gabriel",
		Email: "gabriel@gmail.com",
		Age:   &age,
	}

	result, err := usecases.CreateUser(ctx, user)
	if err != nil {
		fmt.Println(err)
	}

	fmt.Println(result.Email)
	// Output: "gabriel@gmail.com"
	
}

func TestCreateUser(t *testing.T) {
	repo := &mocks.Repositories{}

	usecases := usecases{
		repo: repo,
	}

	ctx := context.Background()
	age := 12

	type Expect struct {
		User  *models.User
		Error error
	}

	tests := []struct {
		Name   string
		Result Expect
		Params *models.User
		Mocks  func()
	}{
		{
			Name: "NameError",
			Result: Expect{
				User:  nil,
				Error: errors.New("name should be defined"),
			},
			Params: &models.User{
				Name:  "",
				Email: "gabriel@gmail.com",
				Age:   &age,
			},
			Mocks: func() {},
		},
		{
			Name: "AgeError",
			Result: Expect{
				User:  nil,
				Error: errors.New("age should be defined"),
			},
			Params: &models.User{
				Name:  "Gabriel",
				Email: "gabriel@gmail.com",
				Age:   nil,
			},
			Mocks: func() {},
		},
		{
			Name: "Error",
			Result: Expect{
				User:  nil,
				Error: errors.New("I'm a error"),
			},
			Params: &models.User{
				Name:  "Gabrielzinho",
				Email: "gabriel@gmail.com",
				Age:   &age,
			},
			Mocks: func() {
				repo.On("CreateUser", ctx, &models.User{
					Name:  "Gabrielzinho",
					Email: "gabriel@gmail.com",
					Age:   &age,
				}).Return(nil, errors.New("I'm a error")).Once()
			},
		},
		{
			Name: "Success",
			Result: Expect{
				User: &models.User{
					ID:    0,
					Name:  "Gabrielzinho",
					Email: "gabriel@gmail.com",
					Age:   &age,
				},
				Error: nil,
			},
			Params: &models.User{
				Name:  "Gabrielzinho",
				Email: "gabriel@gmail.com",
				Age:   &age,
			},
			Mocks: func() {
				repo.On("CreateUser", ctx, &models.User{
					Name:  "Gabrielzinho",
					Email: "gabriel@gmail.com",
					Age:   &age,
				}).Return(&models.User{
					ID:    0,
					Name:  "Gabrielzinho",
					Email: "gabriel@gmail.com",
					Age:   &age,
				}, nil).Once()
			},
		},
	}

	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			test.Mocks()
			result, err := usecases.CreateUser(ctx, test.Params)

			t.Log(result)

			assert.Equal(t, test.Result.User, result, "user should be equal")
			assert.Equal(t, test.Result.Error, err, "error should be equal")
		})
	}
}

// Fazer o teste do outro usecase
// Validar se ID é maior que 0

// func TestGetUser(t *testing.T) {
// 	repo := &mocks.Repositories{}

// 	usecases := usecases{
// 		repo: repo,
// 	}

// 	ctx := context.Background()

// 	type Expect struct {
// 		User  *models.User
// 		Error error
// 	}

// 	tests := []struct {
// 		Name   string
// 		Result Expect
// 		Params int
// 		Mocks  func()
// 	}{
// 		{
// 			Name: "IDError",
// 			Result: Expect{
// 				User:  nil,
// 				Error: errors.New("id should be defined"),
// 			},
// 			Params: 0,
// 			Mocks:  func() {},
// 		},
// 		{
// 			Name: "Error",
// 			Result: Expect{
// 				User:  nil,
// 				Error: errors.New("I'm a error"),
// 			},
// 			Params: 1,
// 			Mocks: func() {
// 				repo.On("GetUser", ctx, 1).Return(nil, errors.New("I'm a error")).Once()
// 			},
// 		},
// 		{
// 			Name: "Success",
// 			Result: Expect{
// 				User: &models.User{
// 					ID:    1,
// 					Name:  "Gabrielzinho",
// 					Email:  "gabriel@gmail.com",
// 					Age:   nil,
// 				},
// 				Error: nil,
// 			},
// 			Params: 1,
// 			Mocks: func() {
// 				repo.On("GetUser", ctx, 1).Return(&models.User{
// 					ID:    1,
// 					Name:  "Gabrielzinho",
// 					Email: "gabriel@gmail.com",
// 					Age:   nil,
// 				}, nil).Once()	
// 			},
// 		},
	


// }
